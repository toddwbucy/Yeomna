...
| Phase 6b | **Built (spec 020)**: `codebase.retire` sweeps what the tree no longer has, taking the file node and the nodes that file declared, and `codebase.prune` sweeps the orphans. Both force-gated, both measured and deleted in one statement, both audited. Retire asks `drift` what is gone rather than deciding for itself, so a present file the analyzer could not read is never retired, which is the guard spec 019's review made necessary. **R23 proposed**: `RetireRequest` gains a `path`, since the captured shape had none and D3 rules that retire removes what the source lost, which needs a tree to know. The dispatch has no phase refusal left |
| Phase 6a | **Built (spec 019)**: `codebase.ingest`, `ingest` (documents and the `conforms` links), `codebase.drift` (D2, writes nothing), and `codebase.validate`, which finds the one invariant the constraints cannot express: an edge whose endpoints live in another graph. Ingestion is a verb now rather than a test, which is what the WeaverTools stand-up was waiting for |

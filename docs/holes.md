...
| Progress | **All seven phases done. The verb layer is complete.** Phase 7 (spec 021, PR #51, merged 2026-09-10): the contract-born CLI tree, where every subcommand path is a wire name with its dots as spaces (D10) and the request is built by deserializing into the closed enum, so no verb has hand-written argument code and the tree cannot lag the contract. The capture's census inverts: a test asserts every wire name is reachable rather than that every command reports its hole. Phase 6 (specs 019 and 020, PRs #47 and #49): ingestion is a verb. `codebase.ingest`, `ingest` with the `conforms` links, `codebase.drift` which writes nothing and reports what moved, `codebase.validate` which finds the one invariant the constraints cannot express, and the destructive pair `codebase.retire` and `codebase.prune`, both force-gated, both measuring and deleting in one statement. Retire asks drift what is gone rather than deciding for itself, which is why 019's review mattered: `missing` is the list retire consumes and it was naming present-but-unassessable files. Phase 5 (spec 018, PR #45): the daemon, H6 filled. Phase 4 (spec 014, PR #36): seven verbs including R18's edge pair, the audit transaction proven by a crash-shaped test, the scoped `sql` verb under R17/R17a, R16's client inside the call lock. Phase 3 (spec 013, PR #34): the graph answers, pruning proven at the verb level, the first destructive verb, the kg pattern stamped from the template under `yeomna_provision`, materialize refusing under R14. Phase 2 (spec 012, PR #32): eleven read verbs, the session, the exhaustive dispatch, and the audit write, with G1 as a permanent test. Phase 1 (spec 010, PR #27): the closed contract, R4 closed, the audit outcome column |
| T3 | **Answered 2026-09-09 (spec 020).** The destructive paths are all verbs now, and the charter's sentence about `codebase retire` leaving no verb-layer record no longer describes this codebase. What stays open is capability rather than surface, and the list has shrunk: `embed.text` answers as of spec 022, so what refuses by name is the `schema.*` verbs (H7) and the `graph-embed.*` verbs (H9). T3 goes false again if one ever ships as a side door. R22 asks separately whether the record is rich enough: the row carries who, when, what was asked, and that it succeeded, while the enumerated sweep rides the envelope and is not durable |
| Open rulings on the last phases | **R23 accepted 2026-09-09** by merging PR #49, which was built on it: `RetireRequest` carries a required `path`, so retire can only sweep what the source truly lacks and a tree it cannot walk is a refusal rather than a licence. **R22 stays open**: whether the audit row should carry the substance of a destructive sweep, not only who asked and that it succeeded. The shape proposed is a `result` jsonb column written by destructive verbs inside the mutation's transaction |
| Next | **The WeaverTools KG stand-up alone**, which is last by Todd's ordering of 2026-09-10 because standing up their graph touches their repository and their deployment and wants a WeaverTools session in the loop rather than this one acting alone. Phase 7 and H8 merged as PR #51, the native embedder is this branch, and hybrid query is spec 023 behind it |
...
**FILLED 2026-09-10 (spec 022, PR #53). The appliance embeds.**
...
| Rulings | R25, R26, and R27 were proposed by the PRD and are implemented as proposed, so they are accepted by merging PR #53, the way R23 was accepted by merging PR #49. They are settled, not open |
| Still open | **R28 needs a ruling**: whether the `code` task wants a query and passage split the way `retrieval` does. The model's snapshot fixes no passage prompt for `code`, so the service uses the query prompt for both halves and says so, and a corpus embedded under the wrong answer costs a re-ingest. Cheap to hold open because R26 put the task on the row, so either way a reader can tell which corpora used the old answer. Then PRD Phase 4: idle unload against the extraction service on the same card, throughput, macro-window late chunking for documents above the ceiling, and M1 |
...
## H8. The analyzer toolchain manager. FILLED 2026-09-10

`tools status` and `tools install`, landed with the CLI tree (spec 021). Status calls the same `resolve_and_probe` the ingest preflight calls, so the two cannot drift on resolution order, and it reports an absent analyzer rather than failing on it, because an appliance without gopls falls back to the structural graph rather than being broken. Install places a binary the operator already has into the managed directory. **R24**: fetching from upstream is not implemented, because a byte arriving from the network into a sealed appliance is a charter section 5 question and not a convenience, and the command says so when asked. Notably the shallowest hole:
...
| | |
|---|---|
| Owner | epic #37, filled by spec 021 |
| CLI holes it fills | 2, both filled |
...
| R22 | A durable record of what a destructive verb swept | **Open, proposed in spec 020.** The audit row carries who, when, which verb, the request's args, and a terminal outcome, so "retire swept 47 nodes under `src/old`" is durable only as "retire was called with that prefix and succeeded". The enumerated sweep rides the response envelope, which nothing persists, and for a regulated buyer that is a gap. The shape that fits is a `result` jsonb column written by destructive verbs only, inside the same transaction as the mutation, so the record of a sweep is as durable as the sweep. A schema change and a contract question, so it waits. Related and now raised three times: no verb reads the audit log |
| R23 | `RetireRequest` carries a path | **Accepted 2026-09-09** by merging PR #49, which the build assumed. The captured shape was `{graph, prefix, force}`, which predates D3's ruling that retire removes nodes whose sources are gone: without a tree the verb takes a prefix on faith and a typo deletes live knowledge. With one it can only sweep what the source truly lacks. Declined alternative on the record: prefix-only retire, coherent as an operator action and consistent with `graph.drop`, and strictly less safe |

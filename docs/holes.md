...
| Next | **Reordered 2026-09-10 by Todd**, with Phase 7 and H8 now done: the native embedder (H4), then hybrid query, and **the WeaverTools KG stand-up last**, because standing up their graph touches their repository and their deployment and wants a WeaverTools session in the loop rather than this one acting alone |
...
| R24 | What `tools install` may reach for | **Proposed in spec 021.** Install places a binary the operator supplies and does not fetch from upstream, because a byte arriving from the network into a sealed appliance is a charter section 5 question rather than a convenience. Implementing a fetch means deciding what the appliance may talk to, how a release is verified, and whether an operator's convenience outranks the seal. The command names the ruling when asked to install without a source |

...
| Open rulings on the last phases | **R22**, a durable record of what a destructive verb swept, and **R23**, `RetireRequest` gaining a `path` since the captured shape had none and D3 rules that retire removes what the source lost. Both proposed in spec 020 and both need Todd. The 020 build assumes R23 |
